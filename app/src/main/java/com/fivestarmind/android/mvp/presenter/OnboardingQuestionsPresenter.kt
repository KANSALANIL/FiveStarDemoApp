...
class OnboardingQuestionsPresenter(private val responseCallBack: ResponseCallBack) {

    private val TAG = OnboardingQuestionsPresenter::class.java.simpleName
    private var error = ErrorResponse()

    interface ResponseCallBack {

        fun onLetsGoResponseSuccess(response: SignInSignUpResponseModel.SignInSignUpResponseFirstModel)

        fun onResponseFailure(errorResponse: ErrorResponse)
    }

    fun hitApiForLetsGo(signUpRequestModel: SignUpRequestModel) {
        ...
        call.enqueue(object : Callback<SignInSignUpResponseModel.SignInSignUpResponseFirstModel> {
            override fun onResponse(
                ...
                if (responseModelSignUp.isSuccessful) {
                    ...
                    Log.d(
                        TAG,
                        "response LetsGo for Body = $myResponse"
                    )
                    responseCallBack.onLetsGoResponseSuccess(responseModelSignUp.body())
